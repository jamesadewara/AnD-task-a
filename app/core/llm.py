    ...
    async def get_completion(
        ...
        for model in models_to_try:
            while True:
                ...
                if not api_key:
                    logger.error("No available API keys for model {model}")
                    break
    ...
    async def get_streaming_completion(
        ...
        for model in models_to_try:
            while True:
                ...
                key_index = self.key_manager.api_keys.index(api_key)
                try:
                    logger.info(f"Streaming LLM attempt: model={model}, key_index={key_index}")
                    import asyncio
                    async def _do_call():
                        async with OpenRouter(api_key=api_key) as client:
                    ...
                    # Check for model congestion/capacity issues
                    if any(m in err_msg for m in ["congested", "overloaded", "503", "capacity", "no provider", "free-models-per-day"]):
                        logger.warning(f"Streaming LLM model {model} congested/limited: {e}. Switching model...")
                        break
                    
                    logger.warning(f"Streaming LLM attempt failed for {model} with key {key_index}: {e}. Trying next key...")
                    continue

            if on_fallback and model != models_to_try[-1]:
                await on_fallback(model, models_to_try[models_to_try.index(model)+1], str(last_error))
